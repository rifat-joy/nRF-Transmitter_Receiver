 ...
 * NRF24 library is required
 * Module <-> NANO (5V,16MHz)
...
*This code receive signal for 7 channels and create a PWM output accordingly
*/
...
#include <nRF24L01.h>
#include <RF24.h>
#include <Servo.h>  //To create PWM signals

const uint64_t pipeIn = 0xE8E8F0F0E1LL;  //This is same in the transmitter
...
void reset_the_Data() {
  // Deafult values to use when NO radio input is detected
...
void setup() {
  //Attaching the servo signal on pins from D2 to D8
  channel_1.attach(2);
  channel_2.attach(3);
  channel_3.attach(4);
  channel_4.attach(5);
  ...
  //reseting the received values
  reset_the_Data();

  //Once again, begining and radio configuration
  radio.begin();
  radio.setAutoAck(false);
  radio.setDataRate(RF24_250KBPS);  // NRF24 datarate Enum from library
  radio.openReadingPipe(1, pipeIn);

  //Starting of the radio comunication
...
void loop() {
  ...
  //To reset if the data if signal is lost for 1 sec.
